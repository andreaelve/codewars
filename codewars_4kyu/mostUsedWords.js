...
// Apostrophes can appear at the start, middle or 
// end of a word ('abc, abc', 'abc', ab'c are all valid)
...
const top_3_words = (string) => {
  let words  = string.toLowerCase().replace(/[^a-z ']|\B'/g, '').replace(/^'$/g, '').split(' ').filter(word => word !== '');
  let matchArray = [];
  words.forEach(el => {
    const subArray = words.filter(word => word == el);
    subArray[0] && matchArray.push(subArray);
    words = words.filter(word => word !== el);
  });
  matchArray = matchArray.sort((a, b) => b.length - a.length);
  return matchArray.length > 3  
    ? [matchArray[0][0], matchArray[1][0], matchArray[2][0]]
    : matchArray.map(el => el[0]);
}

console.log(top_3_words(`In a village of La Mancha, the name of which I have no desire to call to mind, there lived not long since one of those gentlemen that keep a lance in the lance-rack, an old buckler, a lean hack, and a greyhound for coursing. An olla of rather more beef than mutton, a salad on most
